...
- **Caching the *publishable/anon* key in the `flight/supabase` Secrets Manager secret is fine** (M1.1 prereq) — it's public-by-design and it's only there so a new Cowork session recalls the `url`+`anon_key` without a dashboard hunt. **Never** cache the service-role key anywhere, AWS included.
